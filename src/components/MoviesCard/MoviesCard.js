...
function MoviesCard(props) {
  ...
  return (
    <li className="movie">
      <p className="movie__title">isSaved: {props.isSaved.toString()}</p>
      ...
      <button
        ...
        style={{ background: `${props.isSaved ? "red" : "inherit"}` }}
        onClick={!props.isSavedMoviesList ? handleSave : handleRemove}
      >
        <span className={movieButtonTextClassName}>{`${
          props.isSaved ? "Сохранено" : textButton
        }`}</span>
      </button>
    </li>
  );
